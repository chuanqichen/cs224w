...
@torch.no_grad()
def update(model, inx, thresh, adj_t, split_edge, evaluator, batch_size, cpu, debug):
...
def update_similarity(z, upper_threshold, lower_treshold, pos_num, neg_num):
    print("Sample numbers  pos =", pos_num, "  neg =", neg_num)
    print("Z shape", z.shape)
    f_adj = np.matmul(z, np.transpose(z))
    cosine = f_adj
    cosine = cosine.reshape([-1,])
    pos_num = min(round(upper_threshold * len(cosine)), pos_num)
    neg_num = min(round((1-lower_treshold) * len(cosine)), neg_num)
    print("Sample numbers  pos =", pos_num, "  neg =", neg_num)
...
def main():
    ...
    parser.add_argument('-agd','--age_dims', default=[500], type=int, nargs='+', help='List of dimensions in hidden layer')
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.upd == 0:
                ...
                threshold_update = update(model, inx, threshold, adj_orig, split_edge,
