...
class GCNWithAttention(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        self.attention.append(LowRankAttention(self.k, in_channels, dropout, device))
...
def train(model, predictor, emb_rand, emb_node2vec, features, adj_t, split_edge, optimizer, batch_size):
    ...
    x = torch.cat((emb_rand, emb_node2vec(), features), 1)
...
@torch.no_grad()
def test(model, predictor, emb_rand, emb_node2vec, features, adj_t, split_edge, evaluator, batch_size):
    model.eval()
    predictor.eval()
    emb_node2vec.eval()
...
def main():
    ...
    model = GCNWithAttention(args.hidden_channels, args.hidden_channels,
                ...
                args.dropout, args.k, device).to(device)

    features = get_features(adj_t.size(0)).to(device)
    n_features = features.size(1)

    n_node2vec = int((args.hidden_channels - n_features) / 2)
    emb_rand = torch.nn.Embedding(data.num_nodes, args.hidden_channels - n_features - n_node2vec).to(device)
    emb_node2vec = Node2Vec(edge_index, n_node2vec, walk_length=20,
    ...
    for run in range(args.runs):
        ...
        emb_node2vec.reset_parameters()
        model.reset_parameters()
        predictor.reset_parameters()
        optimizer = torch.optim.Adam(
            list(model.parameters()) + list(emb_rand.parameters()) + list(emb_node2vec.parameters()) +
            list(predictor.parameters()), lr=args.lr)

        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, emb_rand.weight, emb_node2vec, features, adj_t, split_edge,
                         optimizer, args.batch_size)

            if epoch % args.eval_steps == 0:
                results = test(model, predictor, emb_rand.weight, emb_node2vec, features, adj_t, split_edge,
