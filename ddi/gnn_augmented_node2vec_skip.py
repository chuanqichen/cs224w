...
def train(model, predictor, emb, features, adj_t, split_edge, optimizer, batch_size):
...
@torch.no_grad()
def test(model, predictor, emb, features, adj_t, split_edge, evaluator, batch_size):
    model.eval()
    predictor.eval()
    emb.eval()
...
def main():
    ...
    emb = Node2Vec(edge_index, args.hidden_channels - n_features, walk_length=20,
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, emb, features, adj_t, split_edge,
                         optimizer, args.batch_size)

            if epoch % args.eval_steps == 0:
                results = test(model, predictor, emb, features, adj_t, split_edge,
